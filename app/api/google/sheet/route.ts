...
export async function GET(req: Request) {
  ...
  try {
    ...
    const formatDate = (d: string) => {
      ...
      if (parts.length !== 3) {
        console.warn(
          `⚠️ Date format error: "${d}" has ${parts.length} parts, expected 3`
        );
    ...
    const startDayShift = new Date(`${formattedDateParam}T08:10:00`);
    const endDayShift = new Date(`${formattedDateParam}T20:10:00`);
    // const startNightShift = endDayShift;
    const endNightShift = new Date(targetDate);
    endNightShift.setDate(targetDate.getDate() + 1);
    endNightShift.setHours(8, 10, 0, 0);
    ...
    const filtered = rows.filter((r) => {
      ...
      if (isNaN(start.getTime())) {
        console.warn(
          `⚠️ Invalid Date created: ${dateStr}T${paddedTime}:00 (original: ${timeStr})`
        );
        return false;
      }
      ...
      if (shift === "day") {
        ...
        const nightStart = new Date(`${formatDate(dateParam)}T20:00:00`);
